...
import com.ibm.cos.v2.auth.credentials.ibmOAuth.IBMOAuthCredentials;
...
public final class CredentialUtils {
    ...
    /**
     * Converts an {@link IBMOAuthIdentity} to IBM credentials
     *
     * <p>Usage of the new IBMOAuthIdentity type is preferred over IBM credentials. But some places may need to still
     * convert to the older IBM credentials type to work with existing code.</p>
     *
     * @param iamCredentialsIdentity The {@link AwsCredentialsIdentity} to convert
     * @return The corresponding {@link AwsCredentials}
     */
    public static IBMOAuthCredentials toIAMCredentials(IBMOAuthIdentity iamCredentialsIdentity) {
        if (iamCredentialsIdentity == null) {
            return null;
        }
        if (iamCredentialsIdentity instanceof BasicIBMOAuthCredentials) {
            return (BasicIBMOAuthCredentials) iamCredentialsIdentity;
        }
        return new BasicIBMOAuthCredentials(iamCredentialsIdentity.apiKey(),iamCredentialsIdentity.serviceInstanceId());
    }
