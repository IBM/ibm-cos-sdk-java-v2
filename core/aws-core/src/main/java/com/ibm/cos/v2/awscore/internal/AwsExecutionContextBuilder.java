...
package com.ibm.cos.v2.awscore.internal;

import static com.ibm.cos.v2.auth.signer.internal.util.SignerMethodResolver.resolveSigningMethodUsed;
...
import static com.ibm.cos.v2.core.internal.useragent.BusinessMetricsUtils.resolveRetryMode;
...
import com.ibm.cos.v2.auth.credentials.ibmOAuth.IBMSignerExecutionAttribute;
...
import com.ibm.cos.v2.awscore.internal.authcontext.AuthorizationStrategy;
import com.ibm.cos.v2.awscore.internal.authcontext.AuthorizationStrategyFactory;
import com.ibm.cos.v2.awscore.util.SignerOverrideUtils;
import com.ibm.cos.v2.core.HttpChecksumConstant;
...
import com.ibm.cos.v2.core.internal.util.HttpChecksumResolver;
import com.ibm.cos.v2.core.signer.Signer;
import com.ibm.cos.v2.core.useragent.BusinessMetricCollection;
...
public final class AwsExecutionContextBuilder {
    ...
    public static <InputT extends SdkRequest, OutputT extends SdkResponse> ExecutionContext
         ...
         *
         * 1. Extract the configured credentials provider from the client configuration.
         * 2. Check if it's a standard AWS credentials provider.
         * 3. If IBM IAM credentials are detected, store them in execution attributes
         *    so that the IBM custom signer can use them.
         * 4. Otherwise, fall back to resolving the signing method for standard AWS credentials.
         */
        IdentityProvider<? extends AwsCredentialsIdentity> provider = clientConfig.option(AwsClientOption.CREDENTIALS_IDENTITY_PROVIDER);
        if (provider instanceof AwsCredentialsProvider) {
            AwsCredentials creds = ((AwsCredentialsProvider) provider).resolveCredentials();
            if (creds instanceof BasicIBMOAuthCredentials) {
                IBMInternalFlag.setIAM(true);
                BasicIBMOAuthCredentials ibmCreds = (BasicIBMOAuthCredentials) creds;
                executionAttributes.putAttribute(IBMSignerExecutionAttribute.IAM_CREDENTIALS, ibmCreds);
                if(ibmCreds.serviceInstanceId() != null) {}
            } else {
