...
import static com.ibm.cos.v2.http.auth.aws.internal.signer.util.CredentialUtils.sanitizeCredentials;
...
import static com.ibm.cos.v2.http.auth.aws.internal.signer.util.SignerConstant.PRESIGN_URL_MAX_EXPIRATION_DURATION;
...
import com.ibm.cos.v2.http.auth.aws.internal.signer.util.CredentialUtils;
...
public final class DefaultAwsV4HttpSigner implements AwsV4HttpSigner {
    ...
    public SignedRequest sign(SignRequest<? extends AwsCredentialsIdentity> request) {
            LOG.debug(() -> "Signing request with DefaultAwsV4HttpSigner");
    ...
    public CompletableFuture<AsyncSignedRequest> signAsync(AsyncSignRequest<? extends AwsCredentialsIdentity> request) {
        LOG.debug(() -> "Signing async request with DefaultAwsV4HttpSigner");
    ...
    private static SignedRequest doSign(SignRequest<? extends Identity> request,
        ...
        V4RequestSigningResult requestSigningResult = null;

        // In case of HMAC → remove IAM-only headers if exists before sending it for signing
        requestBuilder = requestBuilder.removeHeader("ibm-service-instance-id");
        requestBuilder = requestBuilder.removeHeader("ibm-sse-kp-encryption-algorithm");
        requestBuilder = requestBuilder.removeHeader("ibm-sse-kp-customer-root-key-crn");

        requestSigningResult = requestSigner.sign(requestBuilder);
