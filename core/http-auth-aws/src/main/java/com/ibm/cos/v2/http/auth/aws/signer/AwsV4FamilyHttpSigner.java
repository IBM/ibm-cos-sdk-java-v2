...
package com.ibm.cos.v2.http.auth.aws.signer;
...
/**
 * An interface shared by {@link AwsV4HttpSigner}  for defining signer properties that are common
 * across both signers.
 */
@SdkPublicApi
public interface AwsV4FamilyHttpSigner<T extends Identity> extends HttpSigner<T> {
    ...
    SignerProperty<String> IBM_IAM_APIKEY =
        SignerProperty.create(AwsV4FamilyHttpSigner.class, "apiKey");

    SignerProperty<String> IBM_IAM_SERVICEINSTANCEID =
        SignerProperty.create(AwsV4FamilyHttpSigner.class, "serviceInstanceId");
