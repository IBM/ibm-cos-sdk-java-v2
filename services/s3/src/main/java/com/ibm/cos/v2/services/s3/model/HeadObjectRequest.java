...
public final class HeadObjectRequest extends S3Request implements ToCopyableBuilder<HeadObjectRequest.Builder, HeadObjectRequest> {
    ...
    private static final SdkField<String> RESPONSE_CACHE_CONTROL_FIELD = SdkField
            ...
            .getter(getter(HeadObjectRequest::responseCacheControl))
    ...
    private static final SdkField<String> RESPONSE_CONTENT_DISPOSITION_FIELD = SdkField
            ...
            .getter(getter(HeadObjectRequest::responseContentDisposition))
    ...
    private static final SdkField<String> RESPONSE_CONTENT_ENCODING_FIELD = SdkField
            ...
            .getter(getter(HeadObjectRequest::responseContentEncoding))
    ...
    private static final SdkField<String> RESPONSE_CONTENT_LANGUAGE_FIELD = SdkField
            ...
            .getter(getter(HeadObjectRequest::responseContentLanguage))
    ...
    private static final SdkField<String> RESPONSE_CONTENT_TYPE_FIELD = SdkField
            ...
            .getter(getter(HeadObjectRequest::responseContentType))
    ...
    private static final SdkField<Instant> RESPONSE_EXPIRES_FIELD = SdkField
            ...
            .getter(getter(HeadObjectRequest::responseExpires))
     ...
     * <code>416 - Requested Range Not Satisfiable</code> error.
     * </p>
     * 
     * @return HeadObject returns only the metadata for an object. If the Range is satisfiable, only the
     *         <code>ContentLength</code> is affected in the response. If the Range is not satisfiable, S3 returns a
     *         <code>416 - Requested Range Not Satisfiable</code> error.
     */
     ...
     * </p>
     * <p>
     * <b>General purpose buckets</b> - If you enable checksum mode and the object is uploaded with a <a
     * href="https://docs.aws.amazon.com/AmazonS3/latest/API/API_Checksum.html">checksum</a> and encrypted with an Key
     * Management Service (KMS) key, you must have permission to use the <code>kms:Decrypt</code> action to retrieve the
     * checksum.
     * </p>
     * <p>
     * <b>Directory buckets</b> - If you enable <code>ChecksumMode</code> and the object is encrypted with Amazon Web
     ...
     *         <p>
     *         <b>General purpose buckets</b> - If you enable checksum mode and the object is uploaded with a <a
     *         href="https://docs.aws.amazon.com/AmazonS3/latest/API/API_Checksum.html">checksum</a> and encrypted with
     *         an Key Management Service (KMS) key, you must have permission to use the <code>kms:Decrypt</code> action
     ...
     *         <code>kms:GenerateDataKey</code> and <code>kms:Decrypt</code> permissions in IAM identity-based policies
    ...
    public interface Builder extends S3Request.Builder, SdkPojo, CopyableBuilder<Builder, HeadObjectRequest> {
         ...
         *        412 (precondition failed) error.</p>
         *        <p>
         *        If both of the <code>If-Match</code> and <code>If-Unmodified-Since</code> headers are present in the
         *        request as follows:
         *        </p>
         ...
         * <p>
         * <b>General purpose buckets</b> - If you enable checksum mode and the object is uploaded with a <a
         * href="https://docs.aws.amazon.com/AmazonS3/latest/API/API_Checksum.html">checksum</a> and encrypted with an
         * Key Management Service (KMS) key, you must have permission to use the <code>kms:Decrypt</code> action to
         * retrieve the checksum.
         * </p>
         * <p>
         * <b>Directory buckets</b> - If you enable <code>ChecksumMode</code> and the object is encrypted with Amazon
         * Web Services Key Management Service (Amazon Web Services KMS), you must also have the
         * <code>kms:GenerateDataKey</code> and <code>kms:Decrypt</code> permissions in IAM identity-based policies and
         ...
         *        <p>
         *        <b>General purpose buckets</b> - If you enable checksum mode and the object is uploaded with a <a
         *        href="https://docs.aws.amazon.com/AmazonS3/latest/API/API_Checksum.html">checksum</a> and encrypted
         *        with an Key Management Service (KMS) key, you must have permission to use the <code>kms:Decrypt</code>
