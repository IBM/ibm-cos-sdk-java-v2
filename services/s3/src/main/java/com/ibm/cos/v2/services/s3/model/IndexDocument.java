...
public final class IndexDocument implements SdkPojo, Serializable, ToCopyableBuilder<IndexDocument.Builder, IndexDocument> {
    private static final SdkField<String> SUFFIX_FIELD = SdkField
            ...
            .traits(LocationTrait.builder().location(MarshallLocation.PAYLOAD).locationName("Suffix")
                    .unmarshallLocationName("Suffix").build(), RequiredTrait.create()).build();
     ...
     *         the suffix is <code>index.html</code> and you make a request to <code>samplebucket/images/</code>, the
     *         data that is returned will be for the object with the key name <code>images/index.html</code>.) The
     *         suffix must not be empty and must not include a slash character.</p> <important>
     *         <p>
    ...
    public interface Builder extends SdkPojo, CopyableBuilder<Builder, IndexDocument> {
         ...
         * <p>
         * A suffix that is appended to a request that is for a directory on the website endpoint. (For example, if the
         * suffix is <code>index.html</code> and you make a request to <code>samplebucket/images/</code>, the data that
         * is returned will be for the object with the key name <code>images/index.html</code>.) The suffix must not be
         ...
         * 
         * @param suffix
         *        A suffix that is appended to a request that is for a directory on the website endpoint. (For example,
         *        if the suffix is <code>index.html</code> and you make a request to <code>samplebucket/images/</code>,
         *        the data that is returned will be for the object with the key name <code>images/index.html</code>.)
         *        The suffix must not be empty and must not include a slash character.</p> <important>
         *        <p>
         *        Replacement must be made for object keys containing special characters (such as carriage returns) when
         *        using XML requests. For more information, see <a href=
         *        "https://docs.aws.amazon.com/AmazonS3/latest/userguide/object-keys.html#object-key-xml-related-constraints"
